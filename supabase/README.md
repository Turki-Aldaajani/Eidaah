...
## حوكمة رفع المواد (مهمة I2)

رفع المادة للمكتبة العامة يدخل كـ**طلب** (`material_requests`, pending)، وأدمن
يقبل أو يرفض. القبول يُدرج المادة في `materials` (scope=library) فتظهر بالمكتبة (F2).

### التطبيق (مرة واحدة)

1. طبّق [`migrations/20260723100000_i2_governance.sql`](migrations/20260723100000_i2_governance.sql)
   في **SQL Editor** (بعد G1 وS1). يضيف دالة `is_admin()`، ويشدّ صلاحيات
   `materials`: **إدراج مادة المكتبة للأدمن فقط** (يزيل ثغرة G1 المتساهلة)،
   ويمنح الأدمن رؤية/مراجعة كل الطلبات.
2. **عيّن الأدمن**: عدّل قائمة الإيميلات في **موضعين متوافقين**:
   - `is_admin()` داخل ملف الـ migration (الأمان الفعلي — RLS).
   - [`frontend/src/data/admins.js`](../frontend/src/data/admins.js) (إظهار صفحة المشرف بالواجهة).

### التدفق (معيار القبول)

طالب يفتح **«أضف مقرراً»** (`/submit-material`) ← يرسل طلباً ← المشرف يفتح
**`/admin`** ← «قبول» ← المادة تظهر في المكتبة. (معالجة I3 خطوة خادمية لاحقة.)

```bash
cd frontend && npm test -- --watchAll=false governance Admin SubmitMaterial
```
