...
عند إعادة فحص الكتب الفعلية تبيّن التالي:

| المادة | المرحلة | الملف | الحالة | المطلوب |
|---|---|---|---|---|
| رياضيات | م١ | `book-riadiaat-1mm.pdf` | `needs_ocr` (سكان صور، لا نص) | OCR عربي ثم تعبئة الوحدات |
| رياضيات | م٢ | `الرياضيات-ثاني-متوسط.pdf` | `needs_ocr` (سكان صور — صفحة واحدة فيها نص) | OCR عربي |
| رياضيات | م٣ | `book-riadiaat-3mm.pdf` | `needs_ocr` | OCR عربي |
| مهارات رقمية | م١ | `book-raqmia-1mm.pdf` | `needs_ocr` | OCR عربي |
| **مهارات رقمية** | **م٢** | `مهارات-رقمية-ثاني-متوسط.pdf` | **`text_ok` (نص قابل للاستخراج — ١٧٧ صفحة)** | **جاهز للإدخال مباشرة** |
| مهارات رقمية | م٣ | `book-raqmia-3mm.pdf` | `needs_ocr` | OCR عربي |

> **الجاهز الآن:** مهارات رقمية م٢ فقط (نص حقيقي قابل للاستخراج). خمسة كتب لا تزال سكان صور.
> ملاحظة جودة: النص المُستخرَج بترتيب حروف معكوس (RTL بصري)؛ قد يخفض جودة الشرح/التلخيص —
> يُفضَّل OCR أو خطوة تطبيع لاحقة.
> الخط والتحقق (السكيما + منطق المعالجة + الحوكمة ضد الأسماء الوهمية) جاهزة ومختبَرة.
> ما يتبقّى لإكمال معيار القبول لكل مادة عمليّاً: (١) نصّ الكتاب (OCR لخمسة كتب سكان صور)،
> (٢) مفتاح `GROQ_API_KEY`، (٣) مفاتيح Supabase.
